...
从伽利略 1604 年的斜面数据出发，经割线极限进入切线与导函数；参数面板按步骤开放切点与间隔，时间线上的圆环标出值得动手的时刻。
...
当前正式模板共 32 个：十个算法与数据结构案例（滑动窗口、归并排序、快速排序、二分查找、二叉树 BFS，以及栈 · 括号匹配、单调栈 · 下一个更大元素、链表反转、二叉搜索树 · 查找与插入、Dijkstra 最短路径），两个微积分 Gold Template（导数与切线 · 伽利略斜面数据，定积分 · 阿基米德与黎曼和），九个高中圆锥曲线 Gold Template（含椭圆第一课三件套：绳长实验、标准方程推导、a/b/c 与离心率），两个高中物理 Gold Template（抛体运动 · 两颗子弹与一条抛物线，弹簧简谐振动 · 等时性与能量的椭圆），两数之和、氧化还原、DNA 复制、东亚季风，以及面向高校试点的生态学课程包五案例（Logistic 种群增长、兔群与混沌、猞猁与雪兔捕食循环、草履虫竞争排斥、喀拉喀托岛屿生物地理学）。微积分与物理四案例沿用生态学课程包确立的标准重制：真实数据或真实实验开场、逐步机制推理、参数分步生效、模型边界收尾。Gold Case 同时是 `/templates` 中经过人工校验的公开教师案例；对应的 Hidden Benchmark Variant 只保存 eval-only 题目与事实要求，必须经过真实 Pipeline 生成，不能读取公开冻结 Playbook 获得通过。仓库中的其他 fixture 主要用于渲染回归与视觉验收，不等同于已经发布给学习者的成品案例。
...
| 参数互动 | 调整数学参数；修改算法演示的受控输入（如二分查找目标值、滑动窗口大小、BFS 起点）并确定性重放 | 只覆盖 Playbook 中声明的受控字段 |
...
打开 `/templates` 可以体验 29 个静态正式案例；打开 `/create` 进入讲解创建工作台，其中“二分查找”快捷项会直接进入对应静态模板，不创建生成任务。根路径 `/` 是产品 Landing，不是生成器入口。
...
| 管理后台 | 不可用 | ops 子域名 `/admin`（apex `/admin` 已移除，nginx `METAVIEW_OPS_ALLOW_IPS` IP 白名单门）；仅绑定 `METAVIEW_OPS_ADMIN_USER_ID` 的 admin 账户（ops 未配置该项则启动失败） |
...
| `METAVIEW_OPS_ALLOW_IPS` | ops 子域名传输层 IP 白名单（逗号/空格分隔，支持 CIDR；留空 = nginx `deny all`，ops 子域名不可达） |
