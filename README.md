...
当前正式模板共 24 个：五个算法案例（滑动窗口、归并排序、快速排序、二分查找、二叉树 BFS），两个微积分 Gold Template（导数与切线 · 伽利略斜面数据，定积分 · 阿基米德与黎曼和），九个高中圆锥曲线 Gold Template（含椭圆第一课三件套：绳长实验、标准方程推导、a/b/c 与离心率），两个高中物理 Gold Template（抛体运动 · 两颗子弹与一条抛物线，弹簧简谐振动 · 等时性与能量的椭圆），以及两数之和、氧化还原、DNA 复制、东亚季风与面向高校试点的生态学双案例（Logistic 种群增长、兔群与混沌）。微积分与物理四案例沿用生态学课程包确立的标准重制：真实数据或真实实验开场、逐步机制推理、参数分步生效、模型边界收尾。Gold Case 同时是 `/templates` 中经过人工校验的公开教师案例；对应的 Hidden Benchmark Variant 只保存 eval-only 题目与事实要求，必须经过真实 Pipeline 生成，不能读取公开冻结 Playbook 获得通过。仓库中的其他 fixture 主要用于渲染回归与视觉验收，不等同于已经发布给学习者的成品案例。
