...
describe("assetRegistry", () => {
  ...
  it("finds subject assets by semantic role", () => {
    expect(getAssetPack("geography-earth-basic")?.subject).toBe("geography");
    ...
    expect(findAssetByRole("geography", "map_layer")?.id).toBe("east-asia-land-110m");
    ...
    expect(findAssetByRole("physics", "block")?.id).toBe("block-body");
  });
  ...
  it("keeps the public manifest schema aligned with source provenance metadata", () => {
    ...
    expect(schema.required).toEqual(expect.arrayContaining(["schemaVersion", "licenseMode", "sources", "defaultTeachingUse"]));
  ...
  it("ships second-pass internal SVG quality markers for subject assets", () => {
    ...
    expect(readPublicAsset("/assets/metaview-kits/algorithm-code-basic/graph/bfs-graph-preset.json")).toContain(
      '"source": "structured-preset"',
    );
