...
describe("IntakeScreen launch home", () => {
  ...
  it("submits freeform prompts with a null domain when the hint cannot infer one", async () => {
    const { getByPlaceholderText, getByRole, getByText, props } = renderIntake();

    fireEvent.change(getByPlaceholderText(/输入一道数学题/), {
      target: { value: "帮我做一张社团活动宣传海报" },
    });
    expect(getByText("将交给系统自动识别题目类型。")).toBeTruthy();
    fireEvent.click(getByRole("button", { name: "生成讲解" }));
