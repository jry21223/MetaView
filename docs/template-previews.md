...
`/templates` 是模板和正式案例的唯一公共入口。它保留完整的 20 个模板目录，但只有已登记的正式案例可交互；尚未完成的模板必须显示“制作中”并保持禁用，不能退回旧的生成入口。
...
每个默认案例至少有五个实际状态不同的步骤，每一步有 2–3 个固定问题。案例数据必须继续符合 `PlaybookScript`；不要为公开模板引入第二套播放器或渲染协议。
