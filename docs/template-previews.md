...
| `stack-brackets` | 逐字符扫描表达式、左括号入栈、右括号与栈顶配对出栈、匹配对轨道、交叉 / 未闭合 / 多余右括号三种失败、`O(n)` | 表达式预设 |
| `monotonic-stack` | 柱状数组、值递减的下标栈轨道、弹出即写答案的结果轨道、全递减 / 全递增两种极端、`O(n)` | 输入数组预设 |
