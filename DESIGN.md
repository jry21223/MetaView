...
- 块状数组表达位置、索引、连续区间、成员关系和状态转移；
- 柱状数组表达数值大小、相对顺序和交换，负数必须使用真实零轴，不得按绝对值映射高度；
- 窗口、搜索区间、partition 等连续范围属于共享覆盖层，不得退化为若干独立高亮元素；
- deque、结果序列和辅助数组使用独立辅助轨道，不得藏在旁白中，也不得复用“已排序”状态；
- 栈使用竖直槽位列画在主序列右侧：栈底在下、栈顶在上、空槽保持可见、栈顶旁标出 `top`；不得把栈画成横向轨道，那会丢掉“堆叠”这个隐喻；
- 表现方式由主要教学关系决定。不确定时使用块状数组，不能因为元素恰好都是数字就自动使用柱状图。

化学场景（`chemistry_scene`，见 `docs/chemistry-coursepack.md`）：

- 宏观（装置、溶液颜色、仪表读数）、微观（粒子放大图、分子模型）与符号（方程、曲线、能量图）同屏，并随步骤联动；
- 元素使用降饱和的 CPK 色，作为命名常量留在化学 kit 内；同一元素在分子模型、粒子与装置电极中颜色一致；
- 旁观离子弱化显示，本步的主角加聚焦圈；结构、强调与文字颜色仍走语义 token；
- 文字要么位于固定区域，要么交给避障布局；任何文字不得压在原子、键、曲线、装置或其他文字上，由布局测试守住；
- 键的断开与形成、粒子合并、离子迁移用固定帧数的形变表达，不使用弹跳或装饰性粒子。
